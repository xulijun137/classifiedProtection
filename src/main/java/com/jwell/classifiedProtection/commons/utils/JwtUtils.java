...
public class JwtUtils {
    ...
    private final static String JWT_SECRET = "7786df7fc3a34e26a61c034d5ec8245d";

    private final static long TOKEN_TIME = 3 * 24 * 60 * 60 * 1000;//设置登录过期时间为3天(单位：毫秒）
     ...
     * @throws Exception
     */
    public static String createJWT(String id, String subject, User user){
        ...
        claims.put("userJsonString", JSONObject.toJSONString(user));
        ...
        if (TOKEN_TIME >= 0) {
            long expMillis = nowMillis + TOKEN_TIME;
    ...
    public static String updateJWT(String token) {
        try {
            ...
            return createJWT("JWT", subject, null);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
        return "0";
    ...
    public static void main(String[] args) throws Exception {
        ...
        User user = new User();
        user.setId(1);
        user.setUserName("Ronnie");

        String jwt=JwtUtils.createJWT("jwt", "{id:10086,name:Ronnie}", user);
        ...
        //String jwt1="eyJhbGciOiJIUzI1NiJ9.eyJ1aWQiOiJEU1NGQVdEV0FEQVMuLi4iLCJzdWIiOiJ7aWQ6MTAwLG5hbWU6eGlhb2hvbmd9IiwidXNlcl9uYW1lIjoiYWRtaW4iLCJuaWNrX25hbWUiOiJEQVNEQTEyMSIsImV4cCI6MTUxNzgzNTEwOSwiaWF0IjoxNTE3ODM1MDQ5LCJqdGkiOiJqd3QifQ.G_ovXAVTlB4WcyD693VxRRjOxa4W5Z-fklOp_iHj3Fg";
        Claims c=JwtUtils.parseJWT(jwt);//注意：如果jwt已经过期了，这里会抛出jwt过期异常。
        ...
        //JSONObject userJsonObject = JSONObject.parseObject(c.get("userJsonString", String.class));
        User userObject = JSONObject.parseObject(c.get("userJsonString", String.class), new TypeReference<User>() {});
        System.out.println(userObject.getUserName());//DSSFAWDWADAS...
    }
