...
@RestController
@RequestMapping("/app/question-bank")
public class QuestionBankAppController extends BaseController {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "query", name = "assetId", value = "资产Id", dataType = "Integer"),
    ...
    public ResultObject getQuestionBankList(String assetId, Integer pageNum, Integer pageSize) {

        ResultObject resultObject = ResultObject.failure();

        try {
            QueryWrapper<QuestionBank> questionBankQueryWrapper = new QueryWrapper<>();
            questionBankQueryWrapper.lambda().orderByDesc(QuestionBank::getQuestionOrder);
            if (assetId != null) {
                questionBankQueryWrapper.lambda().eq(QuestionBank::getAssetId, assetId);
            }
